(* The syntax and representation of MOCaml patterns.
  ...
  The source language for patterns also supports two shorthands
  for list patterns:
	[p;...;p] and p::p 
  These shorthands are "desugared" by the parser into data constructor
  patterns.  For example, h::t is shorthand for Cons(h,t).
...
The source language for expressions also supports two shorthands
for list expressions:
	[e;...;e] and e::e 
These shorthands are "desugared" by the parser into data constructor
expressions.  For example, 1::[] is shorthand for Cons(1,Nil).
